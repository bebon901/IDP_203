...
void loop(){
 ...
 int valFrontRight = digitalRead(frontRightPin); // read right input value
  ...
  if (valFrontLeft && valFrontRight && !valRight && !valLeft && !stop)
  {
  ...
  if (!valFrontLeft && !valFrontRight && valRight && !stop) // ignoring left sensor!!!
{
  while (!valFrontLeft or !valFrontRight){
      leftMotor->run(FORWARD);
